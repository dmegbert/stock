...
def linear_regression_vars(prices, s1, s2, s3):
    ...
    """
    X = np.empty((len(prices) - 51, 4))
    Y = np.empty(len(prices) - 51)
    for i in range(50, len(prices) - 1):
        dp = prices[i + 1] - prices[i]
        dp1 = predict_dpi(prices[i - 5:i], s1)
        dp2 = predict_dpi(prices[i - 25:i], s2)
        dp3 = predict_dpi(prices[i - 50:i], s3)
     #   r = (v_bid[i] - v_ask[i]) / (v_bid[i] + v_ask[i])
     ## Essentially get rid of r
        r = 0.00025
        X[i - 50, :] = [dp1, dp2, dp3, r]
        Y[i - 50] = dp
    return X, Y
...
def evaluate_performance(prices, dates, dps, t, step):
    ...
    sell_amt = 1
    last_bought = 0
    for i in range(50, len(prices) - 1, step):
        temp = trades
        # long position - BUY
        if dps[i - 50] > t and position <= 0:
            position += buy_amt
            bank_balance -= buy_amt * prices[i]
            trades += 1
            print(dps[i-50])
            last_bought = i
        # short position - SELL
        if dps[i - 50] < -t and position > 0:
            bank_balance += sell_amt * prices[i]
            position -= sell_amt
            trades += 1
            print(dps[i - 50])
        if temp != trades:
            account_balance = (position * prices[i]) + bank_balance
            bank_str = "Running account balance is: " + str(account_balance)
            price_str = "Current price is: " + str(prices[i])
            trades_str = "Current number of trades: " + str(trades)
            position_str = "Positions held: " + str(position)
            date_str = "Trade date: " + str(dates[i])
            print(trades_str)
            print(position_str)
            print(bank_str)
            print(price_str)
    ...
    if position == 1:
        bank_balance += prices[last_bought + 1] * position
